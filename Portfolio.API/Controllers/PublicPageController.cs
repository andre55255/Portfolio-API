    ...
    public class PublicPageController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetMyProjectsAsync()
        {
            try
            {
                ...
                List<ProjectReturnVO> data =
                    await _publicPageService.GetProjectsAsync(requestData);

                return StatusCode(StatusCodes.Status200OK,
                    APIResponseVO.Ok($"Meus projetos foram listados com sucesso", data));
            }
            catch (NotFoundException ex)
            {
                return StatusCode(StatusCodes.Status404NotFound,
            ...
            {
                _logService.Write($"Falha inesperada ao fazer rotina de pegar dados de projetos. Headers: {JsonSerializer.Serialize(Request.Headers)}", this.GetPlace(), ex);

                return StatusCode(StatusCodes.Status500InternalServerError,
                    APIResponseVO.Fail($"Falha inesperada ao fazer rotina de pegar dados de projetos"));
            }
        }
    }
}
